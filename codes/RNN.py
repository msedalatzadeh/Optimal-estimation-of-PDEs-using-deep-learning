# Module for recurrent neural network (RNN) predictor
...
from tensorflow.keras.layers import Dense, Reshape, Conv1D, Flatten, SimpleRNN, TimeDistributed
...
plt.savefig('./figs/RNN-training_ICs-%dmodes.png'%N)
save('./training-data/RNN-training_input-%dmodes.npy'%N, input)
save('./training-data/RNN-training_output-%dmodes.npy'%N, output)
...
###### Save Data Below ##################################
plot_model(model, to_file='./figs/RNN-model_plot.png', show_shapes=True, show_layer_names=True)
model.save('./model/RNN-%s-%s-%s-%dmodes.h5'%(act_function,opt_method,loss_function,N))
anim = animation.FuncAnimation(fig,lambda i: animate(i,dt), init_func=init,frames=int(t_max/dt/4), interval=0.1, blit=False)
anim.save('./mp4s/RNN-%dmodes.mp4'%N, fps=30, extra_args=['-vcodec', 'libx264'])
ff = ffmpy.FFmpeg(inputs = {'./mp4s/RNN-%dmodes.mp4'%N:None} , 
    outputs = {'./gifs/RNN-%dmodes.gif'%N: None})
ff.run()